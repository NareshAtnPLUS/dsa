 ...
 * @param {string} target
 */
const findSmallestSubstring = (str, target) => {
    const result = {};
    let windowStart = 0,
        matched = 0,
        charMap = {};
    ...
    for (let windowEnd = 0; windowEnd < str.length; windowEnd++) {
        ...
        while (matched === Object.keys(charMap).length) {
            ...
            result[windowEnd - windowStart + 1] = str.slice(
                windowStart,
                windowEnd + 1
            );
    ...
    const smallestSubstring = result[Math.min(...Object.keys(result))];
    return smallestSubstring ? smallestSubstring : '';
};
console.log(
    `Anagram exists in start indices: ${findSmallestSubstring(
        'abbcabc',
        'abc'
    )}`
);

console.log('Result :', findSmallestSubstring('adcad', 'abc'));
