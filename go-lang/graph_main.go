...
func graphMain() {
	...
	// pacificAndAtlantic := graphs.PacificAtlanticV2(heights)
	// surroundedRegions := graphs.CaptureSurroundedRegionsV2(board)
	...
	// 	{1, 5},
	// }
	// timeTaken := graphs.RottingOrangesV2(matrix)
	// fmt.Printf("\n Time taken to rot all the oranges: %d", timeTaken)

	// canFinsih := graphs.CourseSchedulerV2(5, prerequisites)
	// fmt.Printf("Can someone finish this courses:%v", canFinsih)

	// redundantConnection := graphs.FindRedundantConnectionV2(edges)
	// fmt.Printf("The rendundant connection is %v", redundantConnection)

	// ladderLength := graphs.LadderLength("hot", "dog", []string{"hot", "dog"})
	...
	// Find the itinerary
	// itinerary := graphs.FindItinerary(tickets)
	...
	// 	{10, 9, 8, 7, 6},
	// }
	// minWaitTime := graphs.SwimWater(grid)
	// fmt.Printf("\n the minimum time the swimmer has to wait to swim from {0,0} to {%d,%d} is %d", len(grid)-1, len(grid[0])-1, minWaitTime)
	// minCost := graphs.MinCostConnectPoints(points)
	// fmt.Println(minCost)
	...
	alienOrder := graphs.AlienOrder(words)
	fmt.Println(alienOrder)
